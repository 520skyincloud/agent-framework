# SDK 与 UI 事件协议 / SDK And UI Event Protocol

## 本章目标 / Goals

定义 CLI、GUI、SDK 和远程客户端共用的事件流与控制消息。

Define shared event streams and control messages for CLI, GUI, SDK, and remote clients.
...
- UI should not wait for a final string.
...
本章位于 `sdk` 层。它不是孤立模块，而是和主循环、工具系统、上下文管理、持久化、权限或 SDK 事件流共同工作。实现时要明确本层是否拥有状态，是否会产生副作用，是否会改变下一轮模型输入。

This chapter belongs to the `sdk` layer. It is not an isolated module; it works with the main loop, tool system, context management, persistence, permissions, or SDK event stream. Implementation must make clear whether this layer owns state, produces side effects, or changes the next model input.
...
建议接口命名保持直接，例如 `sdkConfig`、`sdkState`、`sdkEvent`、`sdkResult`。如果这些类型变得过大，优先拆分所有权，而不是把所有字段塞进一个全局对象。

Use direct names such as `sdkConfig`, `sdkState`, `sdkEvent`, and `sdkResult`. If these types become too large, split ownership instead of putting every field into one global object.
...
本章的重点是把 `sdk` 层变成可实现、可测试、可恢复的工程边界。只要边界清楚，后续实现者就不需要靠猜。

The focus of this chapter is to turn the `sdk` layer into an implementable, testable, and recoverable engineering boundary. When boundaries are clear, implementers do not need to guess.
