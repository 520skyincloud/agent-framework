...
| 配置 | 默认值 | 说明 |
|---|---:|---|
| `contextWindowTokens` | `200_000` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `summaryReserveTokens` | `20_000` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `autoCompactBufferTokens` | `13_000` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `manualReserveTokens` | `3_000` | 实现时显式引用，不要隐藏在业务逻辑中。 |
