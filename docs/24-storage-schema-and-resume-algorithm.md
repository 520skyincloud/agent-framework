...
```text
1. 读取 metadata.json。
2. 按顺序回放 transcript.jsonl。
3. 校验消息 UUID 是否唯一。
4. 校验 tool_use / tool_result 配对是否合法。
5. 读取任务元数据，并把陈旧的运行中任务标记为 unknown 或 stopped。
6. 读取 Agent 元数据和 sidechain 路径。
7. 校验引用到的工具输出文件是否存在。
8. 对缺失文件或孤立配对插入修复型 system message。
9. 重新估算 token 用量。
10. 如果超过阻塞限制，要求先 compact，再进入下一轮用户消息。
```
