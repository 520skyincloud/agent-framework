...
|---|---|---|---:|---|---|---|---|
| `defaultResultPersistChars` | 工具结果落盘阈值 | 字符 | `50_000` | 50k 以上的输出通常是日志、搜索结果或构建输出，直接塞进 prompt 会挤掉用户意图。 | 工具输出超过 50k 字符时写入 `tool-results`，tool_result 中只保留摘要、路径和预览。 | 模型能直接看到更多原文，但上下文更快膨胀。 | 更多结果需要模型按路径重新读取。 |
| `perMessageResultChars` | 单条消息工具结果硬上限 | 字符 | `200_000` | provider 对单条消息体积有限制，200k 是防止一次工具批次过大的硬线。 | 映射 provider 消息前统计全部 tool_result，超过后拆分、落盘或 microcompact。 | 大批结果更少拆分，但请求更容易被 provider 拒绝。 | 更频繁外部化，模型即时可见信息减少。 |
| `previewBytes` | 落盘输出预览大小 | 字节 | `2_000` | 2KB 通常能包含命令、错误头、文件路径和第一段关键输出。 | 工具输出落盘后，将前 2KB 放入 tool_result preview。 | preview 更完整，但每条 tool_result 更长。 | preview 可能看不到真正失败位置。 |
...
```xml
<persisted-output>
输出过大（1.2 MB）。完整输出已保存到：/path/to/tool-results/toolu_123.txt
