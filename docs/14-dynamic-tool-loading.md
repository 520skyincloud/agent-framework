...
说明工具太多时如何延迟 schema、搜索工具并保持 prompt cache 稳定。

Explain how to defer schemas, search tools, and keep prompt cache stable when there are too many tools.
...
- Dynamic loading is a context optimization technique.
...
本章位于 `dynamic-tools` 层。它不是孤立模块，而是和主循环、工具系统、上下文管理、持久化、权限或 SDK 事件流共同工作。实现时要明确本层是否拥有状态，是否会产生副作用，是否会改变下一轮模型输入。

This chapter belongs to the `dynamic-tools` layer. It is not an isolated module; it works with the main loop, tool system, context management, persistence, permissions, or SDK event stream. Implementation must make clear whether this layer owns state, produces side effects, or changes the next model input.
...
建议接口命名保持直接，例如 `dynamic-toolsConfig`、`dynamic-toolsState`、`dynamic-toolsEvent`、`dynamic-toolsResult`。如果这些类型变得过大，优先拆分所有权，而不是把所有字段塞进一个全局对象。

Use direct names such as `dynamic-toolsConfig`, `dynamic-toolsState`, `dynamic-toolsEvent`, and `dynamic-toolsResult`. If these types become too large, split ownership instead of putting every field into one global object.
...
本章的重点是把 `dynamic-tools` 层变成可实现、可测试、可恢复的工程边界。只要边界清楚，后续实现者就不需要靠猜。

The focus of this chapter is to turn the `dynamic-tools` layer into an implementable, testable, and recoverable engineering boundary. When boundaries are clear, implementers do not need to guess.
