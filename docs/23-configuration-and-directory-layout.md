# 配置系统与目录结构 / Configuration And Directory Layout

## 本章目标 / Goals

定义配置优先级、项目目录、用户目录和运行时目录。

Define config precedence, project directories, user directories, and runtime directories.
...
This chapter belongs to the `config` layer. It is not an isolated module; it works with the main loop, tool system, context management, persistence, permissions, or SDK event stream. Implementation must make clear whether this layer owns state, produces side effects, or changes the next model input.
...
Use direct names such as `configConfig`, `configState`, `configEvent`, and `configResult`. If these types become too large, split ownership instead of putting every field into one global object.
...
The focus of this chapter is to turn the `config` layer into an implementable, testable, and recoverable engineering boundary. When boundaries are clear, implementers do not need to guess.
