...
| `grepHeadLimit` | 搜索返回行数上限 | 行 | `250` | 250 行能提供足够线索，同时避免搜索结果刷屏。 | Grep 命中超过上限时截断。 | 搜索更完整，但模型更难从长列表中抓重点。 | 可能漏掉后面的关键命中。 |
| `globMaxResults` | 文件发现数量上限 | 条 | `100` | 100 个路径足够定位范围，同时避免把整个仓库塞进上下文。 | Glob 命中超过上限时截断。 | 发现更多文件但上下文更重。 | 可能需要更精确 pattern。 |
| `staleEditProtection` | 编辑前是否校验文件 hash | 布尔 | `true` | Agent 读文件后到写文件前，用户或其它工具可能改过同一文件，hash 校验能防止覆盖新内容。 | Edit/Write 前比对读取时 hash；不一致时拒绝写入并要求重新读取。 | 若关闭，编辑更顺滑但容易覆盖用户新改动。 | 保持开启会多一次校验，文件变化时需要重新生成补丁。 |
