...
1. 保留模型 tool_use 原始顺序。
2. Read/Grep/Glob 可并发。
3. Edit/Write/危险 Bash 串行。
4. 结果按原始顺序返回。
5. 中断时为未完成工具生成错误结果。
...
~~~ts
async function runToolBatch(input: RunToolBatchInput): Promise<RunToolBatchResult> {
