# 权限与安全 / Permissions And Safety

## 本章目标 / Goals

定义权限模式、规则、Bash 安全策略和审计要求。

Define permission modes, rules, Bash safety policy, and audit requirements.
...
- Permissions decide whether to attempt an action; security limits damage.
...
This chapter belongs to the `permissions` layer. It is not an isolated module; it works with the main loop, tool system, context management, persistence, permissions, or SDK event stream. Implementation must make clear whether this layer owns state, produces side effects, or changes the next model input.
...
Use direct names such as `permissionsConfig`, `permissionsState`, `permissionsEvent`, and `permissionsResult`. If these types become too large, split ownership instead of putting every field into one global object.

## 默认值与关键数字 / Defaults And Numbers

| 配置 / Config | 默认值 / Default | 说明 / Notes |
|---|---:|---|
| `defaultBashTimeoutMs` | `120_000` | 实现时显式引用，不要隐藏在业务逻辑中。 / Reference explicitly in implementation; do not hide it in business logic. |
| `maxBashTimeoutMs` | `600_000` | 实现时显式引用，不要隐藏在业务逻辑中。 / Reference explicitly in implementation; do not hide it in business logic. |
| `progressAfterMs` | `2_000` | 实现时显式引用，不要隐藏在业务逻辑中。 / Reference explicitly in implementation; do not hide it in business logic. |
...
The focus of this chapter is to turn the `permissions` layer into an implementable, testable, and recoverable engineering boundary. When boundaries are clear, implementers do not need to guess.
