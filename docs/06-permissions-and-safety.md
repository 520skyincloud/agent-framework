...
|---|---|---|---:|---|---|---|---|
| `defaultBashTimeoutMs` | 普通 Bash 默认超时 | 毫秒 | `120_000` | 120 秒覆盖常见测试、构建和脚本，同时避免命令长期挂住主循环。 | Bash 未显式 timeout 时使用。 | 长命令更少被中断，但卡住风险上升。 | 短测试可能被误杀。 |
| `maxBashTimeoutMs` | Bash 最大超时 | 毫秒 | `600_000` | 10 分钟允许较长构建，但仍强制上限。 | 用户或模型请求更长 timeout 时截断到该值。 | 长任务更容易跑完，但资源占用更久。 | 大型项目构建更容易失败。 |
| `progressAfterMs` | 进度事件延迟 | 毫秒 | `2_000` | 超过该时间还没结束就告诉 UI 正在运行，避免用户以为卡住。 | 工具运行超过阈值时发 progress。 | UI 更安静但反馈更慢。 | UI 更频繁但可能吵。 |
