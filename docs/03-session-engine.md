...
Explain how sessions accept input, store state, handle aborts, emit events, and resume history.
...
- The session engine owns one conversation lifecycle.
...
This chapter belongs to the `session` layer. It is not an isolated module; it works with the main loop, tool system, context management, persistence, permissions, or SDK event stream. Implementation must make clear whether this layer owns state, produces side effects, or changes the next model input.
...
Use direct names such as `sessionConfig`, `sessionState`, `sessionEvent`, and `sessionResult`. If these types become too large, split ownership instead of putting every field into one global object.
...
The focus of this chapter is to turn the `session` layer into an implementable, testable, and recoverable engineering boundary. When boundaries are clear, implementers do not need to guess.
