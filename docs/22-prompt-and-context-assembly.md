...
```text
1. 始终放入系统提示词和必需工具 schema。
2. 如果存在最新压缩摘要，放入最新压缩摘要。
3. 如果会话记忆已经初始化，放入会话记忆。
4. 在预算内放入项目指令。
5. 按从新到旧的顺序放入最近消息。
6. 用 persisted-output 标记替换大型工具结果。
7. 如果超出预算，丢弃或延后可选工具 schema。
8. 如果仍然超出预算，触发 compact。
9. 如果 compact 已经连续失败 3 次，阻塞下一次模型调用。
```
