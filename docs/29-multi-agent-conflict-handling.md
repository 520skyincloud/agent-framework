# 多 Agent 冲突处理 / Multi-Agent Conflict Handling

## 本章目标 / Goals

定义多 Agent 写文件时的 lease、worktree、冲突检测和合并协议。

Define leases, worktrees, conflict detection, and merge protocol for multi-agent writes.
...
- Multi-agent writes need ownership.
...
本章位于 `conflict` 层。它不是孤立模块，而是和主循环、工具系统、上下文管理、持久化、权限或 SDK 事件流共同工作。实现时要明确本层是否拥有状态，是否会产生副作用，是否会改变下一轮模型输入。

This chapter belongs to the `conflict` layer. It is not an isolated module; it works with the main loop, tool system, context management, persistence, permissions, or SDK event stream. Implementation must make clear whether this layer owns state, produces side effects, or changes the next model input.
...
建议接口命名保持直接，例如 `conflictConfig`、`conflictState`、`conflictEvent`、`conflictResult`。如果这些类型变得过大，优先拆分所有权，而不是把所有字段塞进一个全局对象。

Use direct names such as `conflictConfig`, `conflictState`, `conflictEvent`, and `conflictResult`. If these types become too large, split ownership instead of putting every field into one global object.
...
本章的重点是把 `conflict` 层变成可实现、可测试、可恢复的工程边界。只要边界清楚，后续实现者就不需要靠猜。

The focus of this chapter is to turn the `conflict` layer into an implementable, testable, and recoverable engineering boundary. When boundaries are clear, implementers do not need to guess.
