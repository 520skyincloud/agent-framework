# 预算、速率限制与配额控制 / Budget, Rate Limit, And Quota Control

## 本章目标 / Goals

定义 token、成本、工具、Agent、任务和存储预算。

Define budgets for tokens, cost, tools, agents, tasks, and storage.
...
- Budgets must be checked before calls.
...
This chapter belongs to the `budget` layer. It is not an isolated module; it works with the main loop, tool system, context management, persistence, permissions, or SDK event stream. Implementation must make clear whether this layer owns state, produces side effects, or changes the next model input.
...
Use direct names such as `budgetConfig`, `budgetState`, `budgetEvent`, and `budgetResult`. If these types become too large, split ownership instead of putting every field into one global object.

## 默认值与关键数字 / Defaults And Numbers

| 配置 / Config | 默认值 / Default | 说明 / Notes |
|---|---:|---|
| `maxToolCallsPerTurn` | `30` | 实现时显式引用，不要隐藏在业务逻辑中。 / Reference explicitly in implementation; do not hide it in business logic. |
| `maxAgentSpawnsPerTurn` | `3` | 实现时显式引用，不要隐藏在业务逻辑中。 / Reference explicitly in implementation; do not hide it in business logic. |
| `maxBackgroundTasks` | `8` | 实现时显式引用，不要隐藏在业务逻辑中。 / Reference explicitly in implementation; do not hide it in business logic. |
| `maxSessionBytes` | `1 GB` | 实现时显式引用，不要隐藏在业务逻辑中。 / Reference explicitly in implementation; do not hide it in business logic. |
...
The focus of this chapter is to turn the `budget` layer into an implementable, testable, and recoverable engineering boundary. When boundaries are clear, implementers do not need to guess.
