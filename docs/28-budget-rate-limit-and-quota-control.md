...
| 配置 | 默认值 | 说明 |
|---|---:|---|
| `maxToolCallsPerTurn` | `30` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `maxAgentSpawnsPerTurn` | `3` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `maxBackgroundTasks` | `8` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `maxSessionBytes` | `1 GB` | 实现时显式引用，不要隐藏在业务逻辑中。 |
