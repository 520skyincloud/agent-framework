...
| 配置 | 默认值 | 说明 |
|---|---:|---|
| `readMaxOutputTokens` | `25_000` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `readMaxSizeBytes` | `262_144` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `grepHeadLimit` | `250` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `globMaxResults` | `100` | 实现时显式引用，不要隐藏在业务逻辑中。 |
