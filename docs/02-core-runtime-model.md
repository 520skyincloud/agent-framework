...
```mermaid
stateDiagram-v2
  state "接收用户输入" as AcceptUserInput
  state "持久化用户消息" as PersistUserMessage
  state "准备上下文" as PrepareContext
  state "判断是否压缩" as MaybeCompact
  state "模型流式输出" as ModelStreaming
  state "收集助手消息" as CollectAssistant
  state "没有工具调用" as NoToolUse
  state "存在工具调用" as HasToolUse
  state "停止钩子" as StopHooks
  state "完成" as Completed
  state "执行工具" as ExecuteTools
  state "追加工具结果" as AppendToolResults
  state "重试模型" as RetryModel
  state "触发式压缩" as ReactiveCompact
  state "中止" as Abort
  state "补齐缺失工具结果" as SynthesizeMissingToolResults
