...
| 配置 | 默认值 | 说明 |
|---|---:|---|
| `defaultMaxRetries` | `10` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `baseDelayMs` | `500` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `maxDelayMs` | `32_000` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `streamIdleTimeoutMs` | `90_000` | 实现时显式引用，不要隐藏在业务逻辑中。 |
