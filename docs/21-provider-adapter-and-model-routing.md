...
|---|---|---|---:|---|---|---|---|
| `defaultMaxRetries` | 普通 API 重试次数 | 次 | `10` | 10 次覆盖短暂 429、529、5xx 和网络抖动，同时避免无限请求。 | 可恢复 provider 错误达到 10 次后 fallback 或失败。 | 临时故障恢复概率更高，但等待和费用上升。 | 短暂 provider 抖动更容易暴露给用户。 |
| `baseDelayMs` | 重试退避起点 | 毫秒 | `500` | 500ms 对瞬时错误恢复足够快，又不会立即打满 provider。 | 第一次可恢复错误后等待 500ms，后续指数退避。 | 请求更温和，但恢复变慢。 | 恢复更快，但可能加重限流。 |
| `maxDelayMs` | 普通重试最大退避 | 毫秒 | `32_000` | 32 秒限制交互任务的单次等待，不让用户长时间无反馈。 | 指数退避超过 32 秒时封顶。 | 更适合长故障，但用户等待更久。 | 高峰期更容易连续撞限流。 |
| `streamIdleTimeoutMs` | 流式响应空闲超时 | 毫秒 | `90_000` | 90 秒允许模型长思考和慢流式输出，也能识别断流。 | stream 90 秒无事件时中断并按重试表处理。 | 慢响应更少误杀，但断流发现更晚。 | 长回答更容易被误判 idle。 |
