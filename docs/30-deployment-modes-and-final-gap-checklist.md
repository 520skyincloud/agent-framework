# 部署模式与最终检查清单 / Deployment Modes And Final Gap Checklist

## 本章目标 / Goals

比较 CLI、桌面、SDK、远程和团队平台部署模式，并给出最终清单。

Compare CLI, desktop, SDK, remote, and team deployment modes and provide the final checklist.
...
- Deployment mode changes permissions and persistence.
...
本章位于 `deploy` 层。它不是孤立模块，而是和主循环、工具系统、上下文管理、持久化、权限或 SDK 事件流共同工作。实现时要明确本层是否拥有状态，是否会产生副作用，是否会改变下一轮模型输入。

This chapter belongs to the `deploy` layer. It is not an isolated module; it works with the main loop, tool system, context management, persistence, permissions, or SDK event stream. Implementation must make clear whether this layer owns state, produces side effects, or changes the next model input.
...
建议接口命名保持直接，例如 `deployConfig`、`deployState`、`deployEvent`、`deployResult`。如果这些类型变得过大，优先拆分所有权，而不是把所有字段塞进一个全局对象。

Use direct names such as `deployConfig`, `deployState`, `deployEvent`, and `deployResult`. If these types become too large, split ownership instead of putting every field into one global object.
...
本章的重点是把 `deploy` 层变成可实现、可测试、可恢复的工程边界。只要边界清楚，后续实现者就不需要靠猜。

The focus of this chapter is to turn the `deploy` layer into an implementable, testable, and recoverable engineering boundary. When boundaries are clear, implementers do not need to guess.
