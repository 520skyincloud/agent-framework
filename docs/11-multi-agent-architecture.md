...
| 配置 | 默认值 | 说明 |
|---|---:|---|
| `defaultMaxTurns` | `20` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `forkMaxTurns` | `200` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `mcpWaitMaxMs` | `30_000` | 实现时显式引用，不要隐藏在业务逻辑中。 |
| `mcpPollIntervalMs` | `500` | 实现时显式引用，不要隐藏在业务逻辑中。 |
